            ...
            <div class="container mx-auto px-4">
                <div class="container mx-auto px-4">
                    <div class="flex flex-col sm:flex-row justify-between items-center mb-3">
                        <form action="{{ route('depSearch') }}" method="post" class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                            @csrf
                            @method('post')
                        ...
                        <a href="/admin/departement" class="text-blue-500 hover:text-blue-800 flex items-center space-x-1 mt-3 sm:mt-0">
                ...
                @if($data->isEmpty())
                <div > 
                    <p class="text-red font-bold">Aucun departement n'était trouvé.</p>
                </div>
@else 
                            ...
                            @if($item->deleted != 1)
                                <tr>
                                    ...
                                    <td class="border border-gray-300 px-4 py-2">
                                        <a href="/admin/depedit/{{ $item->id }}" title="Editer"> <i class="fas fa-edit fa-lg"></i> 
                                        </a> 
                                    </td>
                                    <td class="border border-gray-300 px-4 py-2">
                                        <form action="{{ route('dep.delete', $item->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" title="Supprimer"><i class="fas fa-trash fa-lg"></i>     
                                            </button>
                                        </form>
                                    </td>
                                </tr>
